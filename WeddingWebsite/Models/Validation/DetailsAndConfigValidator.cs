...
public class DetailsAndConfigValidator: IDetailsAndConfigValidator
{
    ...
    private void Events_LatestFinishTimeIsLastEvent(IWeddingDetails details) {
        var lastFinishTime = details.Events.Last().End; // required
        var lastFinishDate = details.Events.Last().EndDate;
        ...
        if (details.Events.Any(e => e.End != null && ((e.End > lastFinishTime && e.EndDate == lastFinishDate) || (e.EndDate > lastFinishDate)))) {
            // end time not null, if dates equal compare times, else compare dates
    ...
    private void Events_PreviousEndTimeMatchesWithNextStartTime(IWeddingDetails details) {
        var firstEvent = details.Events.First();
        TimeOnly? previousEndTime = firstEvent.End;
        DateOnly previousEndDate = firstEvent.EndDate;
        string previousVenue = firstEvent.Venue.Name;
        foreach (var ev in details.Events.Skip(1)) {
            if (previousEndTime != null && ev.Start > previousEndTime && ev.Date >= previousEndDate && ev.Venue.Name == previousVenue) {
                Warning($"The event '{ev.Name}' starts at {ev.Start}, but the previous event ends at {previousEndDate}, {previousEndTime}. What's happening during this spare {ev.Date.DayNumber - previousEndDate.DayNumber}, {ev.Start - previousEndTime}? If it's a break, add a new event to represent this period of time. If it's travelling between different venues, ensure the venues are set to different values. If this is an error, set the end time of the previous event to {ev.Start} (or if you set it to null, it will be interpreted as {ev.Start}).");
            }
            previousEndTime = ev.End ?? ev.Start;
            previousVenue = ev.Venue.Name;
            previousEndDate = ev.EndDate;
        }
    }
    ...
    {
        var firstEvent = details.Events.First();
        TimeOnly previousStartTime = firstEvent.Start;
        DateOnly previousStartDate = firstEvent.Date;
        foreach (var ev in details.Events.Skip(1))
        {
            if (ev.Start < previousStartTime && ev.Date <= previousStartDate)
            {
                Error($"The event '{ev.Name}' starts at {ev.Date}, {ev.Start}, which is before the previous event that starts at {previousStartDate},  {previousStartTime}. Events should be defined in chronological order.");
            }
            previousStartTime = ev.Start;
            previousStartDate = ev.Date;
        }
    }
    ...
    {
        var firstEvent = details.Events.First();
        TimeOnly previousEndTime = firstEvent.End ?? firstEvent.Start;
        DateOnly previousEndDate = firstEvent.EndDate;
        foreach (var ev in details.Events)
        {
            var endTime = ev.End ?? ev.Start;
            var evEndDate = ev.EndDate;
            if (endTime < previousEndTime && evEndDate <= previousEndDate)
            {
                Error($"The event '{ev.Name}' ends at {evEndDate}, {endTime}, which is before the previous event that ends at {previousEndDate}, {previousEndTime}. Events should be defined in chronological order.");
            }
            previousEndTime = endTime;
            previousEndDate = evEndDate;
        }
    }
    ...
    private void Events_EndTimeShouldBeAfterStartTime(IWeddingDetails details)
    {
        foreach (var ev in details.Events)
        {
            if (ev.EndDate < ev.Date) { // prev day, throw error
                Error($"The event '{ev.Name}' ends at {ev.EndDate}, {ev.End}, which is before it starts on {ev.Date}, {ev.Start}. The end time must be after the start time.");
            }
            else if (ev.EndDate == ev.Date) { // same day, compare times
